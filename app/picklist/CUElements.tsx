...
export function SelectPickList({ ordering, setTeams, picklists }: SelectPickListPropsI) {
   ...
   return <select defaultValue={-1} onChange={handleChange}>
      <option key={-1} disabled value={-1}>select a picklist</option>
      {picklists.map((picklist, index) => {
         return <option key={index} value={`${index}`}>{picklist.name}</option>
      })}
   </select>
}
